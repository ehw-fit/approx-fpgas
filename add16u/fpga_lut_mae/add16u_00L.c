...
uint64_t add16u_00L(uint64_t a, uint64_t b) {
  int wa[16];
  int wb[16];
  uint64_t y = 0;
  wa[0] = (a >> 0) & 0x01;
  ...
  y |=  (sig_70 & 0x01) << 1; // default output
  y |=  (wb[14] & 0x01) << 2; // default output
  y |=  (sig_98 & 0x01) << 3; // default output
  y |=  (wa[5] & 0x01) << 4; // default output
  y |=  (sig_77 & 0x01) << 5; // default output
