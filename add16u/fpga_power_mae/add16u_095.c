...
uint64_t add16u_095(uint64_t a, uint64_t b) {
  int wa[16];
  int wb[16];
  uint64_t y = 0;
  wa[0] = (a >> 0) & 0x01;
  ...
  y |=  (sig_65 & 0x01) << 3; // default output
  y |=  (wb[6] & 0x01) << 4; // default output
  y |=  (sig_44 & 0x01) << 5; // default output
  y |=  (wa[11] & 0x01) << 6; // default output
  y |=  (sig_36 & 0x01) << 7; // default output
