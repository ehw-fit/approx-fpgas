...
uint64_t add16u_0RG(uint64_t a, uint64_t b) {
  int wa[16];
  int wb[16];
  uint64_t y = 0;
  wa[0] = (a >> 0) & 0x01;
  ...
  int sig_58 = wa[12];
  int sig_59 = 0;
  int sig_62 = ~(sig_59 | wb[14]);
  int sig_67 = 0;
  int sig_69 = wb[14];
  int sig_74 = wa[9];
  int sig_77 = sig_74 | wb[12];
  int sig_78 = ~((int)0);
  int sig_79 = wa[10] | wb[10];
  int sig_80 = wb[6] & wb[10];
  ...
  y |=  (sig_69 & 0x01) << 0; // default output
  y |=  (sig_37 & 0x01) << 1; // default output
  y |=  (sig_74 & 0x01) << 2; // default output
  y |=  (sig_94 & 0x01) << 3; // default output
  y |=  (sig_67 & 0x01) << 4; // default output
  y |=  (sig_58 & 0x01) << 5; // default output
  y |=  (sig_62 & 0x01) << 6; // default output
  y |=  (sig_58 & 0x01) << 7; // default output
  y |=  (wb[3] & 0x01) << 8; // default output
